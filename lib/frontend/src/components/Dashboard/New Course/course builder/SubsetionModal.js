...
const SubsetionModal = ({ modalData, setModalData, add = false, view = false, edit = false }) => {
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 px-8 py-10">
                    ...
                    <div className="flex justify-end">
                        <button
                            disabled={loading}
                            type="submit"
                            className={`bg-yellow-100 py-1 p-3 text-richblack-900 rounded-md ${view ? "hidden" : ""}`}
                        >
                            {loading ? "Loading..." : edit ? "Save Changes" : add ? "Save" : ""}
                        </button>
                    </div>
                </form>
            </div>
